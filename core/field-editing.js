...
function toggleIncrementalToken(){
  // Guided editing owns the current parsed field. This is also required on
  // mobile, where the textarea selection is intentionally stale while the
...
function _focusEditorControl(el, stillValid){
  ...
  // Fallback for older mobile browsers without preventScroll support. Restore
  // once, after focus, instead of fighting the browser/keyboard animation with
  // a chain of delayed scrollTop writes.
...
function fieldPanelInput(input){
  ...
  // Parameters such as F20 in "M103 F20" belong to the existing M function.
  // Preserve them while merely opening/navigating the guided block, but clear
  // them when the M number itself is deliberately changed.
...
function fieldPanelKeydown(e){
  ...
  var f=FM.fields[FM.idx];
  // Match the Android guided editor: a sign key changes only the sign of the
  // active numeric value. It must never replace the selected L/C/CC value.
  if(!e.ctrlKey && !e.altKey && !e.metaKey &&
     _fieldAcceptsSign(f) && (e.key==='+'||e.key==='-')){
    e.preventDefault(); e.stopPropagation();
    _setFieldSign(f,e.key);
    focusFieldPanelInput(true);
    return;
  }
  if(e.key==='Escape'){
...
/* The guided panel has two distinct completion controls: Done/Enter commits,
...
function enterFieldMode(label){
  ...
  // Keep web and Android insertion defaults identical. This applies only to a
  // freshly inserted TOOL CALL; opening an existing line never rewrites it.
  if(label==='TOOL CALL'){
...
function parseExistingLine(lineText, bk){
  ...
  // Special case: CALL LBL has format "CALL LBL <n> [REP <count>]" or the
  // documented compact "REP<count>" spelling, so generic field matching is
  // not sufficient.
  if(bk==='LBL CALL'){
