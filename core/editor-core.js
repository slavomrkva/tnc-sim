// editor-core -- web implementation; Android port intentionally deferred until acceptance.
...
function _endAllEditorInput(options){
  ...
  var hasCtxOwner =
    ...
    !!byId('cyclePicker') || !!byId('apprDepPicker') || !!byId('qpFbarVal');
...
function editorClear(){
  ...
  codeEl.value = begin + '\n' + end;
  // Put the desktop caret at the end of BEGIN so Enter can immediately create
  // the first program block. BEGIN itself remains protected by beforeinput.
...
function tokenFor(f){
  ...
  if(f.type==='num'){
    if(f.val===null || f.val==='') return '';
    var nv=String(f.val);
    // APPR/DEP CT use the sign of R to select the documented side. Keep
    // positive entry natural on every keyboard by serializing an unsigned
    // positive value explicitly as R+..., while a chosen minus stays R-....
