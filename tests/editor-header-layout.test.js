...
assert.ok(blockCount < actionsStart, 'block count must precede the right-aligned button group');
assert.match(styles, /\.editor-panel\{[^}]*container-type:inline-size/);
assert.match(styles, /\.panel-header\{[^}]*display:grid[^}]*grid-template-columns:auto minmax\(0,1fr\) auto auto/);
assert.match(styles, /\.ph-actions\{[^}]*flex-wrap:wrap[^}]*min-width:0/);
assert.match(styles, /\.ph-actions\{[^}]*grid-column:4[^}]*justify-self:end/);
assert.match(styles, /\.ph-actions \.ph-btn\{[^}]*white-space:nowrap/);
assert.doesNotMatch(styles, /@container[^}]+\.ph-blocks\{display:none/);
assert.match(styles, /@container \(max-width:570px\)\{\.panel-header\{grid-template-columns:/);
