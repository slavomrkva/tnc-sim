...
CYCL CALL`);
  ...
  assert.strictEqual(Math.min.apply(null, zValues), 15,
    'standalone Q201 after an empty block must not overwrite the defined cycle depth');
...
mustError('L X+10 F0', /Feed must/, 'validator rejects zero feed');
{
  ...
  const inlineM=H.program('TOOL CALL 1 Z S3000 F500\nL X+10 F500 M3 M8');
  const parsedInlineM=H.parse(inlineM);
  const inlineMove=parsedInlineM.sub.find(s=>s.srcLine===4 && !s.isMseg);
  assert.ok(inlineMove&&inlineMove.spindleOn&&inlineMove.coolantOn,
    'known start-of-block M3/M8 on an L block affect that move');
}
mustError('L X+10 F500 M3 M8 M123', /at most two M functions/i,
  'validator rejects a third embedded M function');
{
  const arbitraryC=H.program('TOOL CALL 1 Z S3000 F500\nL X+25 Y+0 F500\nCC X+0 Y+0\nC X+0 Y+25 DR+ M123 M456');
  assert.ok(!H.validate(arbitraryC).some(p=>p.sev==='err'),
    'validator accepts two arbitrary M functions in a C positioning block');
  const knownC=H.program('TOOL CALL 1 Z S3000 F500\nL X+25 Y+0 F500\nCC X+0 Y+0\nC X+0 Y+25 DR+ M3 M8');
  const parsedC=H.parse(knownC);
  const cArc=parsedC.sub.find(s=>s.srcLine===6&&s.rcGeom&&s.rcGeom.kind==='C');
  assert.ok(cArc&&cArc.spindleOn&&cArc.coolantOn,
    'known start-of-block M3/M8 on a C block affect that arc');
}
mustError('L X+25 Y+0\nCC X+0 Y+0\nC X+0 Y+25 DR+ M3 M8 M123',
  /at most two M functions/i,'validator rejects a third C-block M function');
