...
const aboutEn = read('core', 'theme-toast.js');
const aboutDe = read('web', 'i18n-about-de.js');
const worker = read('service-worker.js');
assert.ok(index.includes('id="footerGuideLink" href="/learn/"'), 'Desktop footer must expose the Learn hub');
assert.ok(!index.includes('id="mtabGuide"'), 'Mobile bottom navigation must remain limited to Editor, 3D and Learn');
assert.ok(!styles.includes('.mtab-bar>a'), 'Mobile bottom navigation must not retain link-only tab styling');
assert.ok(aboutEn.includes('id="aboutGuideLink" href="/learn/"'), 'English About popup must expose the English guide');
assert.ok(aboutDe.includes('id="aboutGuideLink" href="/de/learn/"'), 'German About popup must expose the German guide');
assert.ok(!i18n.includes("'mobile.guide'"), 'Removed mobile Guide tab must not leave an orphan translation');
