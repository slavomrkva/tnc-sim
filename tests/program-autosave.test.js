...
// Leaving Learn from the UI must go through closeLearn() so autosave resumes
// immediately; learnExit() alone leaves Learn open with autosave suspended.
assert.match(learnSource, /onclick="closeLearn\(\)"/,
  'the Learn close control must fully close Learn so autosave resumes immediately');
assert.doesNotMatch(learnSource, /onclick="learnExit\(\)"/,
  'no rendered control may call learnExit() and leave autosave suspended');
